...
int main(int argc, char *argv[])
{
    ...
    // create window
    MPI_Win_create(&rank, sizeof(int), sizeof(int), MPI_INFO_NULL, MPI_COMM_WORLD, &window);

    //passive synchronization + put
    MPI_Win_lock(MPI_LOCK_SHARED, left_rank, 0, window);

    MPI_Put(&number, 1, MPI_INT, left_rank, 0, 1, MPI_INT, window);
